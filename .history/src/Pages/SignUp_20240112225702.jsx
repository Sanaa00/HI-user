...
function SignUp() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      Number: Yup.string().matches(/^[0-9]+$/, 'Invalid serial number'),
      Serial: Yup.string().matches(/^[A-Za-z0-9]{8}$/, 'Invalid serial number'),
      Blood: Yup.string().required(),
      Height: Yup.string().required(),
      Weight: Yup.string().required(),
      Pressure: Yup.string().required(),
      Suger: Yup.string().required(),
      Cancer: Yup.string().required(),
      Name: Yup.string().required(),
      Relationship: Yup.string().required(),
      email: Yup.string().email().required(),
      ContactNumber: Yup.string().matches(/^[0-9]+$/, 'Invalid serial number'),
  ...
  return (
    <div>
      <div className="px-6 sm:px-12 lg:mx-26 md:px-20 pt-10 grid  md:grid-cols-2 items-start gap-20 min-h-screen">
        <Formik>
          <div className="w-full flex  flex-col items-center justify-center md:mt-16 lg:mt-14 xl:mt-28  mt-20 sm:mt-0">
